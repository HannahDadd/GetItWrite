		...
		ScrollView {
			VStack(spacing: 20) {
				...
				StretchedButton(text: "SIGN UP!", action: {
					session.updateUser(newUser: User(id: session.user?.uid ?? "Error", displayName: displayName, bio: bio, photoURL: "", writing: writing, authors: authors, writingGenres: writingGenres, colour: Int.random(in: 0..<GlobalVariables.profileColours.count), rating: 3))
					changePage = true
				})
