...
class Proposal: Hashable {
	...
	internal init(id: String, title: String, typeOfProject: String, blurb: String, genres: [String], timestamp: Timestamp, writerName: String, writerId: String, triggerWarnings: [String]) {
		self.id = id
		self.title = title
...
extension Proposal {
	convenience init?(dictionary: [String: Any], id: String) {
		...
		self.init(id: id, title: title, typeOfProject: typeOfProject, blurb: blurb, genres: genres, timestamp: timestamp, writerName: writerName, writerId: writerId, triggerWarnings: triggerWarnings)
	}
