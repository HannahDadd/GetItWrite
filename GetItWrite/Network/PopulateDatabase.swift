...
extension FirebaseSession {
	...
	func populateDatabaseFakeData() {
		...
		let fakeProposal1 = Proposal(id: "fakeProposal1", title: fakeProject1.title, typeOfProject: "Chapter of Book", blurb: fakeProject1.blurb, genres: fakeProject1.genres, timestamp: Timestamp(), writerName: fakeUser1.displayName, writerId: fakeUser1.id, triggerWarnings: fakeProject1.triggerWarnings, wordCount: 80000, authorNotes: "I've had 3 betas and they've all been really helpful. Hoping to get a few more eyes on it before submitting it to a magazine.")
		Firestore.firestore().collection("proposals").document("fakeProposal1").setData(fakeProposal1.dictionary as [String : Any]) { (err) in }

		let fakeUser2 = User(id: "testUser2", displayName: "Ron Cheesly", bio: "I'm the best writer in the world. When I'm not writing I'm abseiling down the grand caneon or flying to my holiday home in Switzerland.", photoURL: "", writing: "I'm currently writing a memoir, its all about me and my exploits abseiling and how I came to own my holiday home. It's fantastic.", authors: ["William Shakespeare", "George Orwell", "Virginia Wolf"], writingGenres: ["Memoir", "Women's Fiction", "Short Stories"], colour: 3, rating: 0, critiqueStyle: "I like giving overall feedback.")
		...
		let fakeProposal3 = Proposal(id: "fakeProposal3", title: fakeProject3.title, typeOfProject: "Chapter of Book", blurb: fakeProject3.blurb, genres: fakeProject3.genres, timestamp: Timestamp(), writerName: fakeUser3.displayName, writerId: fakeUser3.id, triggerWarnings: fakeProject3.triggerWarnings, wordCount: 60000, authorNotes: "My mystery is about done, just need some more feedback. Looking for someone at a similar stage in their work.")
